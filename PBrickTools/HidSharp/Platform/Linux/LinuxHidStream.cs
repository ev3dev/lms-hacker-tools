                ...
                lock (_inputQueue) {
                    while (true) {
                        ...
                        while (!_shutdown) {
                            ...
                            try {
                                ret = NativeMethods.retry (() => NativeMethods.poll (fds, (IntPtr)1, 250));
                            } finally {
                            ...
                            if (0 != (fds[0].revents & (NativeMethods.pollev.ERR | NativeMethods.pollev.HUP))) {
                                break;
                            }
                            if (0 != (fds[0].revents & NativeMethods.pollev.IN)) {
                                ...
                                if (inputLength > 0 && !_device.ReportsUseID) {
                                    inputLength--;
                                }
                        ...
                        while (!_shutdown && _inputQueue.Count == 0) {
            ...
            try {
                lock (_outputQueue) {
                    while (true) {
                        ...
                        if (!_device.ReportsUseID && outputBytesRaw.Length > 0) {
                            outputBytesRaw = outputBytesRaw.Skip (1).ToArray ();
                        }
