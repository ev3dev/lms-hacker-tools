...
namespace HidSharp.ReportDescriptors.Parser
{
    ...
    public class ReportDescriptorParser
    {
        ...
        {
            Report report;
            if (!TryGetReport (type, id, out report)) {
        ...
        public bool TryGetReport (ReportType type, byte id, out Report report)
        {
            for (int i = 0; i < Reports.Count; i++) {
                report = Reports[i];
                if (report.Type == type && report.ID == id) {
                    return true;
                }
            }
        ...
        static int GetMaxLengthOfReports (IEnumerable<Report> reports)
        {
            ...
            foreach (Report report in reports) {
                length = Math.Max (length, report.Length);
            }
            return length;
        }
        ...
        {
            int delimiter = 0;
            List<uint> designatorValues = new List<uint> ();
            IndexBase designator = IndexBase.Unset;
            List<uint> stringValues = new List<uint> ();
            IndexBase @string = IndexBase.Unset;
            List<uint> usageValues = new List<uint> ();
            IndexBase usage = IndexBase.Unset;

            foreach (KeyValuePair<LocalItemTag, uint> kvp in LocalItemState) {
                switch (kvp.Key) {
                case LocalItemTag.DesignatorMinimum:
                    UpdateIndexMinimum (ref designator, kvp.Value);
                    break;
                case LocalItemTag.StringMinimum:
                    UpdateIndexMinimum (ref @string, kvp.Value);
                    break;
                case LocalItemTag.UsageMinimum:
                    UpdateIndexMinimum (ref usage, kvp.Value);
                    break;

                case LocalItemTag.DesignatorMaximum:
                    UpdateIndexMaximum (ref designator, kvp.Value);
                    break;
                case LocalItemTag.StringMaximum:
                    UpdateIndexMaximum (ref @string, kvp.Value);
                    break;
                case LocalItemTag.UsageMaximum:
                    UpdateIndexMaximum (ref usage, kvp.Value);
                    break;

                case LocalItemTag.DesignatorIndex:
                    UpdateIndexList (designatorValues, delimiter, ref designator, kvp.Value);
                    break;
                case LocalItemTag.StringIndex:
                    UpdateIndexList (stringValues, delimiter, ref @string, kvp.Value);
                    break;
                case LocalItemTag.Usage:
                    UpdateIndexList (usageValues, delimiter, ref usage, kvp.Value);
                    break;
