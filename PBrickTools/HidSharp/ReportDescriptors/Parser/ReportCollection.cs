...
namespace HidSharp.ReportDescriptors.Parser
{
    public class ReportCollection : ReportMainItem
    {
        ...
        public void Clear ()
        {
            ...
            foreach (ReportMainItem child in children) {
                child.Parent = null;
            }
            Type = 0;
        }
        ...
        public IEnumerable<ReportCollection> Collections {
            get {
                foreach (ReportMainItem item in _children) {
                    if (item is ReportCollection) {
                        yield return (ReportCollection)item;
                    }
                }
            }
        }
        ...
        public IEnumerable<ReportSegment> Segments {
            get {
                foreach (ReportMainItem item in _children) {
                    if (item is ReportSegment) {
                        yield return (ReportSegment)item;
                    }
                }
            }
        }
